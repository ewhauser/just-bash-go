...
This page runs gbash compiled to WebAssembly and keeps one persistent session.
...
This demo runs \`gbash\` in the browser through WebAssembly and keeps a persistent session, so filesystem changes and cwd updates survive across commands.
...
This app vendors Vercel's terminal website example and runs gbash compiled to WebAssembly in the browser.

Pieces involved:

- \`examples/website/browser/gbash-browser.ts\`
  A TypeScript compatibility layer that exports \`Bash\` and \`defineCommand\`
- \`examples/website/wasm/main.go\`
  A browser-facing WASM bridge around a persistent gbash session
- \`examples/website/scripts/build-wasm.sh\`
  Builds \`gbash.wasm\` and copies Go's matching \`wasm_exec.js\`
...
This is a vendored terminal website running gbash in the browser via WebAssembly.
