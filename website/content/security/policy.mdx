...
Tracing is configured on the runtime, not in `policy.Config`. When tracing is enabled, gbash emits structured `trace.Event` records using the `gbash.trace.v1` schema and includes session and execution IDs for correlation. See [Tracing and Logging](/docs/api/tracing-and-logging) for the runtime API and trace modes.
