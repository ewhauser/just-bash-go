...
- **`contrib/`** -- optional modules (`awk`, `jq`, `sqlite3`, `yq`, `extras`, `nodejs`) kept separate to avoid pulling optional dependencies into the core import graph
...
The repo uses `go.work` for local development with 8 modules (root + 7 children):

```
.                  contrib/awk     contrib/extras
contrib/jq         contrib/nodejs  contrib/sqlite3
contrib/yq         examples
```
