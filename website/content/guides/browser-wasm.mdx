...
The `@ewhauser/gbash-wasm` package handles building and bundling these artifacts. In the `examples/website` project, `pnpm build` compiles the WASM from source and copies both files into `public/`.
...
The [examples/website](https://github.com/ewhauser/gbash/tree/main/examples/website) directory contains a full Next.js app running gbash in the browser via WASM, including terminal UI integration.
